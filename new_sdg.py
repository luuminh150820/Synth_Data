...
class SyntheticDataGenerator:
    ...
    def _get_faker_provider(self, column_name):
        ...
        try:
            ...
            def provider_func():
                try:
                    # safe_args filtering
                    safe_args = {k: v for k, v in faker_args.items() if not k.startswith('_') and k not in ['self', 'cls', 'locale']}
    ...
    def _generate_gemini_values(self, column_name, target_count):
        ...
        while generated_count < target_count:
            ...
            prompt = f"{base_prompt}. Generate {batch_size} unique examples of this. Provide the output as a simple list, with each item on a new line."
    ...
    def step1_generate_initial_uniques(self):
        ...
        scaling_factor = math.ceil(len(self.original_data) / self.NUM_ROWS) if self.NUM_ROWS > 0 else 1
    ...
    def save_synthetic_data(self, output_path):
        """Save synthetic data to CSV file. (Original, slight modification for Excel)"""
        ...
        try:
            if output_extension == '.xlsx':
                 # Attempt to save as Excel, requires openpyxl
                 try:
                      import openpyxl
                 except ImportError:
                      output_path = os.path.splitext(output_path)[0] + ".csv" # Change path
                      result_df.to_csv(output_path, index=False, encoding='utf-8') # Save as CSV instead
                 else:
                 ...
                 if output_extension != '.csv':
                      print(f"Warning: Output path '{output_path}' does not end in .csv or .xlsx. Saving as CSV.")
