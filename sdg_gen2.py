...
def parse_sdv_constraints(constraints_list, col_name, enhanced_schema):
    ...
    # For SDV 1.18.0, parameters are standardized
    column_param = 'column_name'
    min_param = 'low_value'
    max_param = 'high_value'
        ...
        try:
            ...
            if isinstance(constraint, dict):
                if constraint.get("constraint_type") == "Range":
                    ...
                    constraints.append(ScalarRange(
                        column_name=col_name,
                        low_value=min_val,
                        high_value=max_val
                    ))
            ...
            if isinstance(constraint, str):
                ...
                if constraint_str.lower() == "unique":
                    ...
                    print(f"Warning: ScalarInequality constraints are not supported in this SDV version.")
        except Exception as e:
    ...
    try:
        ...
        for col_name, col_schema in enhanced_schema.items():
            if "sdv_constraints" in col_schema and col_schema["sdv_constraints"]:
                ...
                all_constraints.extend(col_constraints)
        
        # FIX: In SDV 1.18.0, we need to pass the constraints to fit rather than adding them
        # This removes the need to call add_constraints entirely
        print(f"Found {len(all_constraints)} constraints to apply...")
        
        print("Fitting synthesizer to data...")
        # Pass constraints directly to fit() in SDV 1.18.0
        synthesizer.fit(df, constraints=all_constraints)
        
        print(f"Generating {num_rows} rows of synthetic data...")
        # In SDV 1.18.0, we don't need to pass constraints to sample
