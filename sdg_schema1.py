...
# Configuration
INPUT_FILE = "dim_retail_casa_sample_data.csv" 
OUTPUT_SCHEMA_JSON = "enhanced_schema2.json" 
METADATA_JSON = "metadata1.json"
...
def load_data(file_path):
    """Loads data from a CSV or XLSX file."""
    if not os.path.exists(file_path):
        print(f"Error: Input data file not found: {file_path}")
        return None, None
    ...
    try:
        ...
        if file_extension == '.xlsx' or file_extension == '.xls':
            # Handle Excel files
            print(f"Detected Excel file format: {file_extension}")
            try:
                df = pd.read_excel(file_path, dtype=str)  # Read all as string initially
                print(f"Successfully loaded Excel data from {file_path}. Shape: {df.shape}")
            except Exception as e:
                print(f"Error reading Excel file {file_path}: {e}")
                return None, None
        else:
            # Handle CSV files with simple UTF-8/latin-1 fallback
            print(f"Detected CSV file format: {file_extension}")
            try:
                df = pd.read_csv(file_path, encoding='utf-8', low_memory=False, dtype=str)  # Read all as string initially
                print(f"Successfully loaded CSV data with UTF-8 encoding from {file_path}")
            except UnicodeDecodeError:
                print("UTF-8 decoding failed, trying latin-1...")
                try:
                    df = pd.read_csv(file_path, encoding='latin-1', low_memory=False, dtype=str)  # Read all as string initially
                    print(f"Successfully loaded CSV data with latin-1 encoding from {file_path}")
                except Exception as e:
                    print(f"Error reading CSV with latin-1 encoding from {file_path}: {e}")
                    return None, None
...
2.  A suitable Faker provider name (e.g., 'name', 'random_int', 'date', 'unique.random_int', 'regexify'). If spotted a simple pattern from the samples provided, suggest 'regexify'. If samples are long sentences, suggest 'gemini'.
...
def generate_enhanced_schema(df_columns, key_types, stats, relationships, column_inferences, external_metadata=None):
    ...
    enhanced_schema = {
        "metadata": {
            "generated_at": datetime.now().isoformat(), "original_file": INPUT_FILE,
...
def main():
    ...
    df, original_dtypes = load_data(INPUT_FILE)
