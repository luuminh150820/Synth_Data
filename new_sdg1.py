...
def generate_from_regex_pattern(pattern):
    ...
    # If there are alternatives, randomly choose one and process it
    if len(alternatives) > 1:
        chosen_pattern = random.choice(alternatives)
        global last_element_type, last_char_set, last_literal_char
        last_element_type = None
        last_char_set = []
        last_literal_char = ''
        return generate_from_regex_pattern(chosen_pattern)

    # If no top-level '|' was found, process the single pattern
    ...
    if pattern_to_process == r'\d{10}':
        ...
        last_char_set = string.digits
        last_literal_char = ''
        return generated_chars

    # --- Linear Parsing and Generation (Existing Logic) ---
    result = []
    i = 0
    ...
    while i < len(pattern_to_process):
        ...
        if i + 1 < len(pattern_to_process) and pattern_to_process[i] == '\\':
            ...
            # Handle common escaped metacharacters as literals, including escaped pipe
            elif escape_char in '{}[]+*?.|()\\':
                generated_char = escape_char
                char_set = [] # No set for literal
                element_type = 'literal'
            else:
                # Unrecognized escape sequence, treat as literal backslash and next char
                result.append('\\')
                result.append(escape_char)
                last_element_type = 'literal' # Treat as literal \ and char
                last_char_set = []
                last_literal_char = escape_char # The last literal added was escape_char
                i += 2 # Move past both chars
                continue # Skip the i += 1 at the end

            result.append(generated_char)
            last_element_type = element_type
            last_char_set = char_set
            last_literal_char = generated_char if element_type == 'literal' else '' # Store literal char if applicable
        ...
        # --- Handle Character Classes ---
            ...
            while j < len(char_class_content):
                 if j + 2 < len(char_class_content) and char_class_content[j+1] == '-':
                      ...
                      # Ensure start_char and end_char are single characters and in correct order
                      if len(start_char) == 1 and len(end_char) == 1 and ord(start_char) <= ord(end_char):
                           ...
                           # Invalid range, treat characters as literals
            ...
            if end_bracket + 1 < len(pattern_to_process) and pattern_to_process[end_bracket + 1] == '{':
                 ...
                 last_literal_char = '' # Not a literal
                 i = end_bracket + 1 # Move index to the '{'
                 continue # Go to the next iteration to handle the '{'

            # If not followed by {}, add one random character from the class
            if chars_in_class:
                generated_char = random.choice(chars_in_class)
                result.append(generated_char)
                last_element_type = 'char_class'
                last_char_set = chars_in_class # Store the set for potential + or * later
                last_literal_char = '' # Not a literal
            else:
                 # Empty character class [] - generate nothing
                 last_element_type = None # No element generated
                 last_char_set = []
                 last_literal_char = ''
            ...
            try:
                if ',' in count_spec:
                    ...
                    min_count = int(parts[0]) if parts[0].strip() else 0
                    max_count = int(parts[1]) if parts[1].strip() else min_count + 5 # Default max if not provided
                    # Ensure min_count <= max_count
                    if min_count > max_count: min_count, max_count = max_count, min_count # Swap if min > max
                    repeat_count = random.randint(min_count, max_count)
                else:
                    repeat_count = int(count_spec)
            except ValueError:
                 # Invalid count spec, treat {count} as literal
                result.append('{')
                result.append(count_spec)
                result.append('}')
                last_element_type = 'literal' # The whole {count} block is literal
                last_char_set = []
                last_literal_char = '}' # The last literal added was }
                i = close_brace + 1
                continue

            # Determine the character set or literal based on the *last_element_type*
            char_set_for_repetition = []
            is_literal_repetition = False
            element_to_repeat = '' # For literal repetition

            if last_element_type in ['escape_d', 'escape_w', 'escape_s', 'char_class']:
                 char_set_for_repetition = last_char_set # Use the stored set
                 is_literal_repetition = False
            elif last_element_type == 'literal':
                 # If the last element was a literal, repeat that literal character
                 if last_literal_char != '': # Ensure there was a stored literal character
                      element_to_repeat = last_literal_char
                      is_literal_repetition = True
                      if result and result[-1] == element_to_repeat:
                           result.pop()
                 else:
                      result.append('{')
                      last_element_type = 'literal'
            ...
            # Now, generate 'repeat_count' characters
            generated_chars = []
            if not is_literal_repetition and char_set_for_repetition:
                 if not char_set_for_repetition: # Handle empty char set (e.g., empty class [])
                      pass # Generate nothing for empty set repetition
                 else:
            ...
            if is_literal_repetition:
                last_literal_char = '' # The result of repetition is not a single literal

            i = close_brace + 1
            continue # Go to the next iteration

        # --- Handle other repetition operators: +, *, ? ---
        # These apply to the *immediately preceding* element that was generated.
        elif pattern_to_process[i] in '+*?':
             operator = pattern_to_process[i]

             # Apply the operator to the *last generated element*.

             if last_element_type is None:
                 # Operator at the beginning or after something that generated no output
                 i += 1
                 continue

             # Determine the character set or literal based on the last generated element's type
             char_set_for_repetition = []
             is_literal_repetition = False
             element_to_repeat = ''
             ...
             if last_element_type in ['escape_d', 'escape_w', 'escape_s', 'char_class']:
                  ...
                  if last_literal_char != '': # Ensure there was a stored literal character
                       ...
                       # Should not happen if last_element_type is 'literal' but last_literal_char is empty
                       i += 1
                       continue
             ...
             if operator == '?':
                 if random.random() < 0.5 and result:
                     if len(result) > 0:
                          result.pop() # Remove the last character generated


             elif operator == '*':
                 # Zero or more: add 0-5 more
                 repeats = random.randint(0, 5)
                 if not is_literal_repetition and char_set_for_repetition:
                      # Regenerate random characters from the stored set
                      for _ in range(repeats):
                           if char_set_for_repetition: # Handle empty set
                                result.append(random.choice(char_set_for_repetition))
                 elif is_literal_repetition:
                      # Repeat the stored literal character
                      result.extend([element_to_repeat] * repeats)
                 # last_element_type/last_char_set/last_literal_char already reflect the element type, no need to update here


             elif operator == '+':
                 # One or more: add 1-5 more (the first one is already in result)
                 repeats = random.randint(0, 4) # Add 0 to 4 more
                 if not is_literal_repetition and char_set_for_repetition:
                      for _ in range(repeats):
                           if char_set_for_repetition: # Handle empty set
                                result.append(random.choice(char_set_for_repetition))
                      ...
                      # Repeat the stored literal character
                      result.extend([element_to_repeat] * repeats)
                 # Same logic for updating state as for '*'
        ...
        # --- Handle Regular Character ---
            ...
            last_element_type = 'literal'
            last_char_set = []
            last_literal_char = literal_char # Store the literal character
            i += 1
            continue
